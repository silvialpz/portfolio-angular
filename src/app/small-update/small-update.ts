...
@Component({
  ...
  template: `
  <ng-container>
    <article *ngIf="type === 'life'" class="p-2 rounded-lg flex w-full items-center cursor-pointer"> 
    ...
    <article *ngIf="type === 'techy'" class="p-2 rounded-lg flex w-full items-center cursor-pointer"> 
      <img [src]="imageUrl" alt="{{ title }}" class="w-auto h-25 object-contain rounded-md" /> 
      <div class="flex-grow pl-4"> 
        <p class="text-base/5 font-semibold sans-serif-subtitle-text">{{ title }}</p>
      </div>
    </article>
  </ng-container>
  `,
